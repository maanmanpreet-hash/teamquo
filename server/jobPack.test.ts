...
describe("job pack html", () => {
  it("renders wall drawings, recorded product details, and internal material totals", () => {
    ...
    expect(html).toContain("TV: 75&quot;");
    expect(html).toContain("TV bracket included");
  ...
  it("does not show tv backdrop detail lines on fireplace rows with stale itemDetails", () => {
    const html = generateJobPackHtml(
      {
        id: 12,
        userId: 1,
    ...
    expect(html).not.toContain("TV: 86&quot;");
    expect(html).not.toContain("Backdrop: 2420 x 1220 mm");
    expect(html).not.toContain("TV bottom AFFL: 700 mm");
  });
});
